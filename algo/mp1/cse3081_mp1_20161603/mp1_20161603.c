...
int mss_pow6(int** matrix, int m, int n) {
   ...
   // 4 loops for setting upper-left and lower-right pos of rectangles
      ...
      for(from->col = 0 ; from->col < n ; from->col++) {
         for(to->row = from->row ; to->row < m ; to->row++) {
            for(to->col = from->col ; to->col < n ; to->col++) {
               // adding up all elements in the rectangle.
               sum = 0;
...
int mss_pow4(int** matrix, int m, int n) {
   ...
   // set left column and right column
   for(int left = 0 ; left < n ; left++) {
      ...
      for(int right = left ; right < n ; right++) {
         ...
         // naive solution for mss in 1D array
         int arr_sum;
         for(int i=0 ; i<m ; i++) {
            arr_sum = 0;
            for(int j=i ; j<m ; j++) {
...
int mss_pow3(int** matrix, int m, int n) {
   ...
   for(int left = 0 ; left < n ; left++) {
      ...
      for(int right = left ; right < n ; right++) {
         ...
         // Using Kandane's algorithm to get MSS for 1-D array
